...
//     */
    public static void main(String[] args) throws InterruptedException {
        String regex = "(?<STag><)[/\\?\\s]*(?<Prefix>\\w*:)*(?<TagName>\\w*)\\s*(?<Attributes>(?<Attribute>((?<AttributePrefix>\\w*)\\s*:\\s*)*(?<AttributeName>\\w*)\\s*=\\s*(?<AttributeValue>\"[^\"]*\"|\'[^\']*\'|[^>\\s]*)\\s*)*)\\s*/?(?<ETag>>)";
