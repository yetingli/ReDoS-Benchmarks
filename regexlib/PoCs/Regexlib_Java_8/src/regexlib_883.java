...
public class regexlib_883 {
    ...
    public static void main(String[] args) throws InterruptedException {
        String regex = "(?<expo>public\\:|protected\\:|private\\:) (?<ret>(const )*(void|int|unsigned int|long|unsigned long|float|double|(class .*)|(enum .*))) (?<decl>__thiscall|__cdecl|__stdcall|__fastcall|__clrcall) (?<ns>.*)\\:\\:(?<class>(.*)((<.*>)*))\\:\\:(?<method>(.*)((<.*>)*))\\((?<params>((.*(<.*>)?)(,)?)*)\\)";
        for (int i = 0; i < 1000; i++) {
            ...
            for (int j = 0; j < i * 10000; j++) {
                attackString.append("public: void __thiscall ");
            }
            // 后缀
